...
create table if not exists public.hosts (
  ...
  -- regardless of how many properties they run. Structured fields (rather
  -- than one free-text block) so notification emails can render a clean
  -- address and it's easier to spot a garbage/incomplete entry at a glance.
  address_line text,
  city text,
  state text,
  zip_code text,
...
alter table public.hosts drop column if exists mailing_address;
alter table public.hosts add column if not exists address_line text;
alter table public.hosts add column if not exists city text;
alter table public.hosts add column if not exists state text;
alter table public.hosts add column if not exists zip_code text;
alter table public.hosts add column if not exists country text;
...
-- One-time physical QR Kit purchases. Stripe remains the payment source of
-- truth; this table gives the admin a durable fulfillment queue.
...
-- Property snapshots belonging to a physical QR Kit order. One order may
-- contain multiple properties while qr_kit_orders retains the lead property.
